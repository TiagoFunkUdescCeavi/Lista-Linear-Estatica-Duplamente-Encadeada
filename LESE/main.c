...
void inserir10000Elementos(lista_t *ll) {
  int r = 0;
  for (int i = 0; i < N_INSERE; i++) {
    InserirInicio(ll, &i, &r);
    printf("Inserindo, i: %d, r: %d\n", i, r);
  }
}

void remover3000Elementos(lista_t *ll) {
  int *r = malloc(sizeof(int));
  int resultado;
  for (int i = 0; i < N_REMOVE; i++) {
    r = RemoverInicio(ll, &resultado);
...
int main(int argc, char **argv) {
  ...
  lista_t *ll = NULL;
  ll = CriarLista(sizeof(int), 10000, &Resultado);
